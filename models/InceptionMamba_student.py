...
class Inception2(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.branch1 = BasicConv2d(in_channels, branch_channels, 1, 0)

        self.branch2 = nn.Sequential(
            BasicConv2d(in_channels, branch_channels // 2, 1, 0),
            BasicConv2d(branch_channels // 2, branch_channels, 3, 1)
        )

        self.branch3 = nn.Sequential(
            BasicConv2d(in_channels, branch_channels // 2, 1, 0),
            BasicConv2d(branch_channels // 2, branch_channels, 3, 1),
            BasicConv2d(branch_channels, branch_channels, 3, 1)
        )

        self.branch4 = nn.Sequential(
            nn.AvgPool2d(3, stride=1, padding=1, count_include_pad=False),
            BasicConv2d(in_channels, branch_channels, 1, 0)
        )
...
class SS_Conv_SSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.inception = Inception2(in_channels=hidden_dim//2, out_channels=hidden_dim//2)
        self.attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),  # 全局平均池化
            nn.Conv2d(hidden_dim // 2, hidden_dim // 2, 1),  # 1x1卷积生成注意力权重
    ...
    def forward(self, input: torch.Tensor):
        ...
        input_left = self.inception(input_left)
        attention = self.attention(input_left)
        input_left = input_left * attention
...
InceptionMamba = VSSM(depths=[2, 2, 4, 2],dims=[96,192,384,768],num_classes=6).to("cuda")


data = torch.randn(1,3,224,224).to("cuda")

print(InceptionMamba(data).shape)
