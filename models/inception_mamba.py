...
class PatchEmbedding(nn.Module):
    """Non-overlapping patch projection with LayerNorm + positional embed."""
...
class InceptionMambaEncoder(nn.Module):
    """
    Paper-accurate encoder adapted for SALT Dense Distillation.
    Maintains 196 patches (no hierarchical merging).
    """
    def __init__(
        self,
        patch_size: int = 16,
        ...
        super().__init__()
        self.patch_embed = PatchEmbedding(patch_size, embed_dim, 224)
        ...
        self.blocks = nn.ModuleList([
            InceptionMambaBlock(embed_dim, drop_path=dpr[i]) for i in range(depth)
        ])
    ...
    def forward(self, x: torch.Tensor, return_patches: bool = False,
        ...
        if mask_ratio > 0.0 and self.training:
            ...
            x = torch.where(mask.unsqueeze(-1),
                            self.mask_token.expand(B, L, D), x)
