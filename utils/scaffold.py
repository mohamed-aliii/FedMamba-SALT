...
def compute_control_variate_update(
    ...
    """
    True-Gradient Option II control variate update (AdamW-safe).

    Instead of backing out the gradient from parameter displacement
    (which only works for SGD and explodes with AdamW), we use the
    exact accumulated raw gradients from the local training loop:
        c_i+ = sum(g) / K

    Args:
        encoder, classifier: Client models AFTER local training.
        c_local: Client's current control variate.
        accumulated_grads: Dictionary of summed raw gradients over K steps.
        K: Total local gradient steps.
    ...
    for name, param in encoder.named_parameters():
        key = f"enc.{name}"
        if param.requires_grad and key in c_local:
            c_new = accumulated_grads[key] / float(K)
